	...
	return {
		...
		AtmosphereRequest : function(options) {
			...
			function _supportWebsocket() {
				return _request.webSocketImpl != null || window.WebSocket
						|| window.MozWebSocket;
			}

			/**
			 * Check if server side events (SSE) is supported (check for custom
			 * implementation provided by request object or browser
			 * implementation).
			 * 
			 * @returns {boolean} True if web socket is supported, false
			 *          otherwise.
			...
			function _local(request) {
				var trace, connector, orphan, name = "atmosphere-"
					...
					storage : function() {
						...
						var storage = window.localStorage, get = function(key) {
							...
							storage.setItem(name + "-" + key, jQuery
									.stringifyJSON(value));
						};

						return {
							init : function() {
								set("children", get("children")
										.concat([ guid ]));
								jQuery(window).on(
										"storage.socket",
										function(event) {
				...
				return {
					open : function() {
						...
						if (parentOpened) {
							// Firing the open event without delay robs the user
							// of the opportunity to bind connecting event
							// handlers
							setTimeout(function() {
			...
			function share() {
				var storageService, name = "atmosphere-" + _request.url, servers = {
					...
					windowref : function() {
						...
						var neim = name.replace(/\W/g, ""), win = (jQuery('iframe[name="'
								+ neim + '"]')[0] || jQuery(
								'<iframe name="' + neim + '" />').hide()
				...
				function leaveTrace() {
					...
					// Opera's JSON implementation ignores a number whose a last
					// digit of 0 strangely
					// but has no problem with a number whose a last digit of 9
					// + 1
			 ...
			 * 
			 * @param request
			 *            {Object} request Request parameters, if undefined
						...
						if (rq.reconnect) {
							if (rq.maxRequest == -1
								...
								var skipCallbackInvocation = _trackMessageSize(
										data, rq, _response);
								if (!skipCallbackInvocation) {
									_prepareCallback(_response.responseBody,
											"messageReceived", 200,
											rq.transport);
								}
								...
								jQuery.atmosphere.log(_request.logLevel,
										[ "AJAX reconnect maximum try reached "
												+ _request.requestCount ]);
								_onError(0, "maxRequest reached");
							}
						}
			...
			function _buildWebSocketUrl() {
				...
				return decodeURI(jQuery('<a href="' + url + '"/>')[0].href
						.replace(/^http/, "ws"));
			}
			...
			function _executeSSE(sseOpened) {
				...
				if (_request.enableProtocol && sseOpened) {
					...
					_request.lastTimestamp = Number(_request.stime)
							+ Number(time);
				}
				...
				_sse.onmessage = function(message) {
					if (message.origin != window.location.protocol + "//"
							+ window.location.host) {
						jQuery.atmosphere.log(_request.logLevel,
								[ "Origin was not " + window.location.protocol
										+ "//" + window.location.host ]);
						return;
					}
					...
					var skipCallbackInvocation = _trackMessageSize(message,
							_request, _response);
					if (!skipCallbackInvocation) {
						_invokeCallback();
						_response.responseBody = '';
			...
			function _executeWebSocket(webSocketOpened) {
				...
				_websocket.onclose = function(message) {
					...
					jQuery.atmosphere.warn("Websocket closed, reason: "
							+ reason);
					jQuery.atmosphere.warn("Websocket closed, wasClean: "
							+ message.wasClean);
			...
			function _handleProtocol(request, message) {
				...
				var b = true;
				if (jQuery.trim(message) != 0 && request.enableProtocol
			...
			/**
			 * Track received message and make sure callbacks/functions are only
			 * invoked when the complete message has been received.
			 * 
			 * @param message
			 * @param request
			 * @param response
			...
			function _trackMessageSize(message, request, response) {
				...
				if (request.trackMessageLength) {
					...
					var messageStart = message
							.indexOf(request.messageDelimiter);
					while (messageStart != -1) {
						messageLength = jQuery.trim(message.substring(
								messageLength, messageStart));
						message = message.substring(messageStart
								+ request.messageDelimiter.length,
								message.length);
					...
					if (messages.length == 0
							|| (messageStart != -1 && message.length != 0 && messageLength != message.length)) {
					...
					if (messages.length != 0) {
						response.responseBody = messages
								.join(request.messageDelimiter);
			...
			function _reconnectWithFallbackTransport(errorMessage) {
				...
				if (typeof (_request.onTransportFailure) != 'undefined') {
					...
					jQuery.atmosphere
							.onTransportFailure(errorMessage, _request);
				}

				_request.transport = _request.fallbackTransport;
				var reconnectInterval = _request.connectTimeout == -1 ? 0
						: _request.connectTimeout;
				if (_request.reconnect && _request.transport != 'none'
			 ...
			 *            _request object will be used.
			 * 
			 * @returns {Object} Request object, if undefined, _request object
			 *          will be used.
			 * @private
			 */
			function _attachHeaders(request, url) {
				...
				jQuery.each(rq.headers, function(name, value) {
					var h = jQuery.isFunction(value) ? value.call(this, rq,
							request, _response) : value;
					if (h != null) {
			...
			function _executeRequest(request) {
				...
				if ((rq.transport == 'jsonp')
						|| ((rq.enableXDR) && (jQuery.atmosphere
								.checkCORSSupport()))) {
					_jsonp(rq);
					return;
				}
				...
				if (jQuery.browser.msie && jQuery.browser.version < 10) {
					if ((rq.transport == 'streaming')) {
						rq.enableXDR && window.XDomainRequest ? _ieXDR(rq)
								: _ieStreaming(rq);
						return;
					}
				...
				if (rq.reconnect
					...
					ajaxRequest.onreadystatechange = function() {
						...
						if (jQuery.browser.opera && rq.transport == 'streaming'
								&& rq.readyState > 2
						...
						if ((!rq.enableProtocol || !request.firstMessage)
						...
						if (update) {
							// MSIE 9 and lower status can be higher than 1000,
							// Chrome can be 0
							var status = 0;
							if (ajaxRequest.readyState != 0) {
								status = ajaxRequest.status > 1000 ? 0
										: ajaxRequest.status;
							}
							...
							if (jQuery.trim(responseText.length) == 0
													...
													function() {
														if (_response.status != 500
															...
															try {
																...
																_response.headers = parseHeaders(ajaxRequest
							...
							if (rq.suspend) {
								_response.state = _response.status == 0 ? "closed"
										: "messageReceived";
							} else {
					...
					if (rq.logLevel == 'debug') {
						jQuery.atmosphere.log(rq.logLevel,
			...
			function _doRequest(ajaxRequest, request, create) {
					...
					if (request.connectTimeout > -1) {
						request.id = setTimeout(function() {
							if (request.requestCount == 0) {
								_clearState();
								_prepareCallback("Connect timeout", "closed",
										200, request.transport);
							}
				...
				if (!_request.dropAtmosphereHeaders) {
					...
					if (request.lastTimestamp != undefined) {
						ajaxRequest.setRequestHeader("X-Cache-Date",
				...
				jQuery.each(request.headers, function(name, value) {
					var h = jQuery.isFunction(value) ? value.call(this,
							ajaxRequest, request, create, _response) : value;
					if (h != null) {
			...
			function _reconnect(ajaxRequest, request, force) {
				if (force || request.transport != 'streaming') {
					if (request.reconnect || (request.suspend && _subscribed)) {
						...
						_response.reason = status == 0 ? "Server resumed the connection or down."
								: "OK";

						var reconnectInterval = (request.connectTimeout == -1) ? 0
								: request.connectTimeout;
			...
			function _configureXDR(request) {
				...
				var rewriteURL = rq.rewriteURL
						|| function(url) {
							...
							var match = /(?:^|;\s*)(JSESSIONID|PHPSESSID)=([^;]*)/
									.exec(document.cookie);
				...
				xdr.onerror = function() {
					// If the server doesn't send anything back to XDR will fail
					// with polling
					if (rq.transport != 'polling') {
						_onError("XDR error");
					}
				...
				var handle = function(xdr) {
					...
					var skipCallbackInvocation = _trackMessageSize(message, rq,
							_response);
				...
				return {
					...
					close : function() {
						xdr.abort();
						_prepareCallback(xdr.responseText, "closed", 200,
								transport);
					}
				};
			}
								...
								.iterate(function() {
									try {
										...
										var readResponse = function() {
											// Clones the element not to disturb
											// the original one
											var clone = res.cloneNode(true);

											// If the last character is a
											// carriage return or a line feed,
											// IE ignores it in the innerText
											// property
											// therefore, we add another
											// non-newline character to preserve
											// it
											clone.appendChild(cdoc
													.createTextNode("."));

											var text = clone.innerText;

											text = jQuery.text.substring(0,
													text.length - 1);
											return text;
										...
										if (!jQuery.nodeName(res, "pre")) {
											// Injects a plaintext element which
											// renders text without interpreting
											// the HTML and cannot be stopped
											// it is deprecated in HTML5, but
											// still works
											var head = cdoc.head
													|| cdoc
			...
			function _intraPush(message) {
				...
				try {
					if (_localStorageService) {
						...
						_storageService.signal("localMessage", jQuery
								.stringifyJSON({
									id : guid,
									event : message
								}));
			 ...
			 * 
			 * @param {string,
			 *            Object} Message to send. This is an object, string
				...
				try {
					if (_request.dispatchUrl != null) {
						data = _request.webSocketPathDelimiter
								...
								+ _request.webSocketPathDelimiter + msg;
					} else {
						data = msg;
					}
			...
			function _readHeaders(xdr, request) {
				...
				try {
					...
					var tempUUID = xdr
							.getResponseHeader('X-Atmosphere-tracking-id');
			...
			function _f(response, f) {
				...
				case "unsubscribe":
				case "closed":
					var closed = typeof (_request.closed) != 'undefined' ? _request.closed
							: false;
			...
			function _invokeCallback() {
				...
				var messages = (isString && _request.trackMessageLength) ? (_response.messages.length > 0 ? _response.messages
						: [ '' ])
				...
				for ( var i = 0; i < messages.length; i++) {
					...
					_response.responseBody = (isString) ? jQuery
							.trim(messages[i]) : messages[i];
					...
					if (jQuery.atmosphere.callbacks.length > 0) {
						if (_request.logLevel == 'debug') {
							jQuery.atmosphere.debug("Invoking "
									+ jQuery.atmosphere.callbacks.length
									+ " global callbacks: " + _response.state);
						}
						try {
					...
					if (typeof (_request.callback) == 'function') {
						if (_request.logLevel == 'debug') {
							jQuery.atmosphere
									.debug("Invoking request callbacks");
						}
						try {
			...
			function _disconnect() {
				if (_request.enableProtocol) {
					var query = "X-Atmosphere-Transport=close&X-Atmosphere-tracking-id="
							+ _request.uuid;
					var url = _request.url.replace(/([?&])_=[^&]*/, query);
					url = url
							+ (url === _request.url ? (/\?/.test(_request.url) ? "&"
									: "?")
									+ query
									: "");
			...
			function _clearStorage() {
				...
				if (_storageService != null) {
					...
					_storageService.signal("close", {
						reason : "",
						heir : !_abordingConnection ? guid : (_storageService
								.get("children") || [])[0]
					});
		...
		checkCORSSupport : function() {
			...
			// Force Android to use CORS as some version like 2.2.3 fail
			// otherwise
			...
			if (storage) {
				try {
					...
					return window.StorageEvent
							&& !jQuery.browser.msie
							&& !(jQuery.browser.mozilla && jQuery.browser.version
									.split(".")[0] === "1");
				} catch (e) {
				}
			}
